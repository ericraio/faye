...
class Chatango
	def initialize( name, password, server="c1.chatango.com", port="5222", authid=nil)
		@serv = server;
		@port = port;
		if( authid )
			@name = nil;
  ...
  def authenticate
    @auid = Chatango.request_auth_id( @name, @password )
  end
	...
	def do_add(name=@name)
		@sock.write("wladd:#{name}\r\n\x00")
	end

	def do_rem(name=@name)
		@sock.write( "wldelete:#{name}\r\n\x00" )
	end

	def do_init_conversation(name=@name)
	...
	def do_get_profile_info( name=@name )
